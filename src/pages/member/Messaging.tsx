...
export default function TaskMessages() {
  ...
  const [initialMessagesLoaded, setInitialMessagesLoaded] = useState(false); // only for first load
    ...
    const fetchMessages = async () => {
      try {
        ...
        setMessages((prev) => {
          ...
          if (newMsgs.length === 0) return prev; // no change
          return [...prev, ...newMsgs]; // append only new messages
        });
